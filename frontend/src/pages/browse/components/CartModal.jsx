...
function CartModal({ 
 ...
 // FIXED: Proceed to Campaign Selection instead of Checkout
 const handleProceedToCampaignSelection = () => {
 if (!cart || cart.length === 0) {
 alert('Your cart is empty. Please add some spaces before proceeding.');
 return;
 }
 
 console.log('🛒 Proceeding to Campaign Selection with cart:', cart);
 
 // Transform cart items to the format expected by Campaign Selection
 const selectedSpaces = cart.map(cartItem => {
 const space = cartItem.space || cartItem;
 
 return {
 id: cartItem.spaceId || cartItem.id,
 name: getAreaName(space),
 title: space.title || getAreaName(space),
 baseRate: cartItem.pricePerDay || space.baseRate || 0,
 rateType: space.rateType || 'DAILY',
 duration: cartItem.duration || 7,
 dates: {
 start: new Date().toISOString(),
 end: new Date(Date.now() + (cartItem.duration || 7) * 24 * 60 * 60 * 1000).toISOString()
 },
 totalPrice: cartItem.totalPrice || 0,
 propertyId: space.propertyId || space.property?.id || cartItem.propertyId,
 type: space.type || space.spaceCategory,
 city: space.city || space.property?.city,
 state: space.state || space.property?.state,
 ...
 console.log('🔄 Transformed spaces for Campaign Selection:', selectedSpaces);
 
 // Store in sessionStorage for Campaign Selection to read
...
>
 ...
 {/* FIXED: Now navigates to Campaign Selection */}
 <Button
 onClick={handleProceedToCampaignSelection}
