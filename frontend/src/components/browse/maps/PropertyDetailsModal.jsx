...
>
 ...
 <Button variant="outline" size={20} onClick={() => onUnlistProperty(property.id)} className="rounded-xl border-[hsl(var(--warning))] text-[hsl(var(--warning))] hover:bg-[hsl(var(--warning))]/5 transition-brand">
 <EyeOff className="w-4 h-4 mr-2" /> Unlist
 </Button>
 <Button variant="destructive" size={20} onClick={() => onDeleteProperty(property.id)} className="rounded-xl transition-brand">
...
>
 ...
 {!selectMode && (
 <Button size={20} onClick={() => onBookArea(area)} className="text-xs btn-gradient rounded-xl shadow-sm hover:shadow-[var(--shadow-brand)] transition-brand">Book Now</Button>
 )}
 </div>
 </div>
 </div>
